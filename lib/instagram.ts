...
export async function getMedia(maxPosts = 100): Promise<IgMedia[]> {
  const fields =
  ...
  while (out.length < maxPosts) {
    ...
    const page = await igFetch("/me/media", params);
...
export async function getMediaInsights(mediaId: string): Promise<Partial<IgInsights>> {
  const metricSets = [
...
export async function getStories(): Promise<IgStory[]> {
  const res = await igFetch("/me/stories", {
...
export async function getStoryInsights(storyId: string): Promise<Partial<IgStoryInsights>> {
  ...
  for (const metric of metricSets) {
    try {
      const res = await igFetch(`/${storyId}/insights`, {
        metric,
...
export async function getComments(mediaId: string, limit = 25): Promise<IgComment[]> {
  try {
    const res = await igFetch(`/${mediaId}/comments`, {
...
export async function refreshToken(): Promise<void> {
  ...
  await setSetting("ig_token", json.access_token);
  await setSetting("ig_token_fetched_at", new Date().toISOString());
}
